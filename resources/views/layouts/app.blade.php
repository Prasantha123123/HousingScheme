...
<div class="min-h-screen flex" x-data="{ openSidebar:false }">
  ...
  <aside
    ...
    <nav class="flex-1 px-2 space-y-1">
      ...
      {{-- Logout in sidebar (visible everywhere) --}}
      <form method="POST" action="{{ route('logout') }}" class="mt-4 border-t pt-4">
        @csrf
        <button type="submit" class="w-full text-left px-3 py-2 rounded-lg hover:bg-gray-100">Logout</button>
      </form>
    </nav>
  </aside>
  ...
  <main class="flex-1">
    ...
    <div class="sticky top-0 bg-white border-b px-4 py-3 flex items-center gap-3 z-50">
      ...
      {{-- Filters: dropdown on mobile, inline on md+ --}}
      <div class="relative md:static" x-data="{ openFilters:false }">
        ...
        {{-- Panel (mobile absolute, desktop inline) --}}
        <div id="filter-panel"
             x-cloak
             x-show="openFilters"
