...
@php
  ...
  $prevUnpaid = max(0, $b->billAmount - ($sewerage + $usage*$unitPrice));
@endphp
<div class="bg-white rounded-lg p-4 mb-3 border">
  <div class="flex items-center justify-between">
    <div class="font-medium">{{ $b->month }} · House {{ $b->houseNo }}</div>
  ...
  <dl class="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm mt-2">
    ...
    <div><dt class="text-gray-500">Usage</dt><dd>{{ $usage }} × {{ number_format($unitPrice,2) }}</dd></div>
    <div><dt class="text-gray-500">Carry Forward</dt><dd>{{ number_format($prevUnpaid,2) }}</dd></div>
  ...
  <div class="mt-2 text-sm">
    Receipt: <a target="_blank" href="{{ asset('storage/'.$b->recipt) }}" class="text-blue-600 hover:underline">Open</a>
  </div>
  @endif
</div>
@empty
  <x-alert type="info">No bills yet.</x-alert>
@endforelse
